...
class Welcome extends CI_Controller {
	...
	public function index()
	{
		...
		$data = array(
			...
			"kkm" => $this->db->query("SELECT nama, AVG(total) as total FROM ( SELECT tm_siswa.nama, AVG(tr_nilai.nilai) as total FROM tm_siswa JOIN tr_nilai ON tr_nilai.id_siswa = tm_siswa.id_siswa GROUP BY tm_siswa.id_siswa UNION SELECT tm_siswa.nama, AVG(tr_nilai_sikap.nilai) as total FROM tm_siswa JOIN tr_nilai_sikap ON tr_nilai_sikap.id_siswa = tm_siswa.id_siswa GROUP BY tm_siswa.id_siswa ) temp GROUP BY nama ORDER BY nama asc LIMIT 10")->result()
		);
